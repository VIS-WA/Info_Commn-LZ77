...
void mlp(int sized)//1a) returns the code for the letter pointer
{
	...
	if(c==-1)
	{
		//if the character isn't present in window,it's code is returned here
		righ++;
		printf("(0,%c)\n",temp[0]);
		pointer++; 
	}
	...
	{
		//	c=window.find(temp);
		j=righ-c-lef+1; //stores the index of 1st occurenece of the word
		z=0;
		//cout<<lef<<" "<<righ<<" "<<c<<endl;
		//	cout<<window<<endl;
		lol=righ;
